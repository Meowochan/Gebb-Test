...
const Showtimes = ({ movie, onClose }) => {
  ...
  const ReservationAdjust = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:8000/adjust-reservation', {
        time: localTime,
        title: movie.title,
        ...
        {selectedDate && (
          <div className="showtime-details">
            ...
            {movie.showtimes
              ...
              .map((showtime, index) => (
                <button onClick={() => handleTimeClick(showtime)} key={index} className={`btn min-h-0 h-10 ${selectedTime === showtime ? 'bg-green-500 text-white' : 'bg-gray-200 text-black'}`}>
